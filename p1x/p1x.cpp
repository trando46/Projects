...
// MODIFY: none
// OUTPUT: none
void updateText(BookList &, MemberList &, RatingList &);
// This function allows the user to update the new data to a different
// text file for ratings and books
// IN: getting addresses of the MemberList, BookList, and RatingList's class
//           arrays and an integer number for the user account number.
// MODIFY: none
// OUTPUT: none
...
void mainMenu(MemberList &memberList, BookList &bookList, RatingList &ratingList) {
    ...
    if(option == 4 ){
        updateText(bookList,memberList,ratingList);
...
void updateText(BookList &bookList, MemberList &memberList,
                RatingList &ratingList){

    //Declare the variables
    string fileBook,fileRating;

    //Updating books data
    cout << "Enter the file path for the updated data for books: ";
    cin >> fileBook;

    ofstream myBook(fileBook);
    if(myBook.is_open()){
        for(int i = 1; i <= bookList.sizeBook(); i++){
            myBook << bookList.getBook(i) << endl;
        }
    }
    myBook.close();

    //Updating ratings data
    cout << "Enter the file path for the updated data for ratings: ";
    cin >> fileRating;

    ofstream myRating(fileRating);
    if(myRating.is_open()){
        for(int i = 0; i < memberList.sizeMember(); i++){
            myRating << memberList.get(i+1);
            myRating << endl;
            for(int j = 0; j <= ratingList.sizeRating(); j++){
                myRating << ratingList.getMemberRating(i,j) << " ";
            }
            myRating << endl;
        }
    }
